...
contract auction
{
  // Variables:
    ...
    function set_default_price(uint price) public //7k gas
    {
    ...
    function change_auctioneer_address_easily_for_testing(address payable _auctioneer) public // 5k gas
    {
    ...
    function registerAndCommit(bytes32 h) public payable // first time 49k gas 32kgas
    {
    ...
    {
        require(committed_bids[msg.sender] != 0,"You are not registerd in auction."); //74K
        ...
        require(bid >= 0, "Invalid input, bid >= 0 allowed."); // **** change bid >= 0 to bid > 0. If some one's bid = 0, she can't withdraw her default_price.
        require(sha256(abi.encodePacked(bid, _nonce))==committed_bids[msg.sender], 
    ...
    function pickWinner() public 
    {
        ...
        bids[auctioneer] = bids[bigest_bid_address]; // **** This line of code was added so that the auctioneer 
                                                     // can withdraw the money and the money is not stuck in the contract.
    ...
    function withdraw() public // 17K gas
    {
...
// I was looking at the code of exercise 2 and I noticed that it has two bugs. 
// By changing line 96 and adding line 121, I corrected them and explained them with a comment.